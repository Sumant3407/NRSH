"""
Video management service with Supabase support
"""
...
class VideoService:
    ...
    async def save_video(
        ...
        if self.db_service:
            try:
                video_type_enum = VideoType.BASE if video_type == "base" else VideoType.PRESENT
                ...
                print(f"Warning: Could not save to database: {e}. Using file-based storage.")
    ...
    async def delete_video(self, video_id: str):
        ...
        # Fallback: Remove from metadata file
        with open(self.metadata_file, 'r') as f:
            all_metadata = json.load(f)
