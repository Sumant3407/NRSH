...
class DatabaseService:
    ...
    def _verify_connection(self):
        ...
        try:
            ...
            print(
                "Make sure your Supabase credentials are correct and tables are created."
            )
    ...
    async def get_video(self, video_id: str) -> Optional[VideoModel]:
        """Get video by ID"""
        result = (
            self.client.table(TABLES["videos"]).select("*").eq("id", video_id).execute()
        )
    ...
    async def delete_video(self, video_id: str) -> bool:
        ...
        result = (
            self.client.table(TABLES["videos"]).delete().eq("id", video_id).execute()
        )
        return len(result.data) > 0

    async def create_analysis(
        self, analysis_id: str, base_video_id: str, present_video_id: str
    ) -> AnalysisModel:
    ...
    async def update_analysis_status(
        ...
        result = (
            self.client.table(TABLES["analyses"])
            .update(update_data)
            .eq("id", analysis_id)
            .execute()
        )
    ...
    async def list_analyses(
        self, status: Optional[AnalysisStatus] = None, limit: int = 100
    ) -> List[AnalysisModel]:
    ...
    async def get_detections(
        ...
        query = (
            self.client.table(TABLES["detections"])
            .select("*")
    ...
    async def create_report(
        self, report_id: str, analysis_id: str, file_path: str, file_size: int
    ) -> ReportModel:
    ...
    async def get_report(self, report_id: str) -> Optional[ReportModel]:
        """Get report by ID"""
        result = (
            self.client.table(TABLES["reports"])
            .select("*")
            .eq("id", report_id)
            .execute()
        )
    ...
    async def create_road_segment(
        self, segment_id: str, name: Optional[str], gps_coords: List[List[float]]
    ) -> RoadSegmentModel:
    ...
    async def get_road_segment(self, segment_id: str) -> Optional[RoadSegmentModel]:
        ...
        result = (
            self.client.table(TABLES["road_segments"])
            .select("*")
            .eq("id", segment_id)
            .execute()
        )
