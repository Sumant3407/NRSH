...
class AnalysisService:
    ...
    - Align base/present frames when comparing two videos
    - Run change-detection between matched frames
    - Aggregate results, persist summaries, and update analysis status

    Note: methods use async APIs from video/frame extraction and database
    services; errors are bubbled up to callers and are also written to
    status files for debugging and UI consumption.
    """
    ...
    async def run_single_detection(
        ...
        """Run detection on a single media file (video or image)"""
        ...
        try:
            media_info = await self.video_service.get_video_info(video_id)
            if not media_info or "file_path" not in media_info:
                raise ValueError(f"Media {video_id} not found")

            media_path = Path(media_info["file_path"])
            ...
            if self.db_service:
                try:
                    await self.db_service.create_analysis(
                        ...
                        present_video_id=video_id,
                    )
                except Exception as e:
                    print(f"Warning: Could not create analysis in database: {e}")

            await self._update_status(analysis_id, "processing", 10)
            frames = await self.frame_extractor.extract_frames(media_path)

            await self._update_status(analysis_id, "processing", 60)
            detections = []
            for frame, metadata in frames:
                dets = await self.detector.detect(frame)
                for det in dets:
                    det["frame_number"] = metadata.frame_number
                    det["timestamp"] = getattr(metadata, "timestamp", 0.0)
                    detections.append(det)

            await self._update_status(analysis_id, "processing", 85)
            aggregated_results = await self._aggregate_results(detections, None)
    ...
    async def run_analysis(
        ...
        try:
            ...
            if not base_info:
                raise ValueError(f"Base video {base_video_id} not found")
            if not present_info:
                raise ValueError(f"Present video {present_video_id} not found")

            if "file_path" not in base_info:
                raise ValueError(f"Base video {base_video_id} missing file_path")
            if "file_path" not in present_info:
                raise ValueError(f"Present video {present_video_id} missing file_path")

            base_path = Path(base_info["file_path"])
            present_path = Path(present_info["file_path"])

            if not base_path.exists():
                raise FileNotFoundError(f"Base video file not found: {base_path}")
            if not present_path.exists():
                raise FileNotFoundError(f"Present video file not found: {present_path}")
            ...
            aligned_pairs = await self.frame_aligner.align_frames(
                base_frames,
                present_frames,
                base_info.get("gps_data"),
                present_info.get("gps_data"),
            )
                    ...
                    for det in detections:
                        try:
                            element_type = ElementType(
                                det.get("element_type", "pavement_crack")
                            )
    ...
    async def _update_status(
        self, analysis_id: str, status: str, progress: int, error: Optional[str] = None
    ):
    ...
    async def get_detections(self, analysis_id: str) -> List[Dict]:
        ...
        if self.db_service:
            try:
                ...
                print(
                    f"Warning: Could not fetch detections from database: {e}. Using file-based storage."
                )
    ...
    async def _aggregate_results(
        self, detections: List[Dict], road_segments: Optional[List[Dict]] = None
    ) -> Dict:
    ...
    def _generate_summary(self, aggregated_results: Dict) -> Dict:
        ...
        total_issues = sum(agg["total"] for agg in aggregated_results.values())
