...
- *Supported uploads:** You can upload either videos (e.g., `.mp4`, `.mov`) or still images (e.g., `.jpg`, `.png`). Pair your Base and Present inputs using the same type (image→image or video→video) for best results.

- *Single-media detection:** Toggle “Single-media detection” in the uploader to run detections on one image or video without comparison; results appear in the dashboard like standard analyses.
...
## Download sample media (optional)
...
This downloads example videos to `uploads/samples/`:
- `base_road_sample.mp4`
- `present_road_sample.mp4`
- `base_gps.json` and `present_gps.json` (optional)

For image-based testing, supply your own paired images (Base and Present) in formats like `.jpg` or `.png`, then upload them through the web UI.
